...
void writeListFromFile(const char *file, List *list) {
    ...
    if (strstr(file, "users_info.txt") != NULL) {
        ...
        while (fscanf(fp, "%s %s %s %d %d %d %s %s %s %d %lld %d %s %d\n", user->account, user->password,
                      user->phone_number, &user->user_type, &user->receive_status, &user->send_status, user->friend[0],
                      user->friend[1], user->friend[2], &user->delivery_leave, &user->time, &user->try_times,
                      user->message, &user->message_status) != EOF) {
            listAdd(list, user);
    ...
    // 2. 快递员表
        ...
        while (fscanf(fp, "%s %s %d %lld %d\n", courier->account, courier->password, &courier->status, &courier->time,
                      &courier->try_times) != EOF) {
    ...
    else if (strstr(file, "platform_warehouse.txt") != NULL || strstr(file, "users_send.txt") != NULL ||
             strstr(file, "shelf_") != NULL || strstr(file, "admin_warehouse.txt") != NULL ||
             strstr(file, "users_push.txt") != NULL || strstr(file, "couriers_push.txt") != NULL) {
        Package *package = (Package *)malloc(sizeof(Package));
        memset(package, 0, sizeof(Package));

        while (fscanf(fp, "%s %s %s %d %d %lf %lf %d %lf %s %s %lld %d %s\n", package->package_id,
                      package->receiver_account, package->courier_account, &package->pick_up_code, &package->isExpress,
                      &package->volume, &package->weight, &package->special_type, &package->value,
                      package->sender_account, package->shelf_id, &package->time, &package->rejected,
                      package->remark) != EOF) {
    ...
    // 5. 平台表
        ...
        while (fscanf(fp, "%s %s %lld %d\n", platform->account, platform->password, &platform->time,
                      &platform->try_times) != EOF) {
    ...
    if (strstr(file, "users_info.txt") != NULL) {
        while (current != NULL) {
            ...
            fprintf(fp, "%s %s %s %d %d %d %s %s %s %d %lld %d %s %d\n", user->account, user->password,
                    user->phone_number, user->user_type, user->receive_status, user->send_status, user->friend[0],
                    user->friend[1], user->friend[2], user->delivery_leave, user->time, user->try_times, user->message,
                    user->message_status);
            current = current->next;
        }
    } else if (strstr(file, "admins_info.txt") != NULL) {
        ...
        while (current != NULL) {
            ...
            fprintf(fp, "%s %s %s %d %d %lf %lf %d %lf %s %s %lld %d %s\n", package->package_id,
                    package->receiver_account, package->courier_account, package->pick_up_code, package->isExpress,
                    package->volume, package->weight, package->special_type, package->value, package->sender_account,
                    package->shelf_id, package->time, package->rejected, package->remark);
            current = current->next;
        }
    } else if (strstr(file, "platforms_info.txt") != NULL) {
        while (current != NULL) {
